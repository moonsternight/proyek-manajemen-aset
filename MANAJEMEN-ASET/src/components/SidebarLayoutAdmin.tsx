...
const SidebarLayoutAdmin: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const handleLogout = async () => {
    const alreadyLoggedOut = sessionStorage.getItem("hasLoggedOut");

    if (!alreadyLoggedOut) {
      try {
        await addActivityLog("LOGOUT");
        sessionStorage.setItem("hasLoggedOut", "true");
        sessionStorage.removeItem("hasLoggedOut");
      } catch (err) {
        console.error("❌ Gagal mencatat log logout:", err);
      }
    }
          ...
          >
            <Link
              ...
              style={{
                ...
                borderRadius: "8px",
                background:
                  isActive("/admin-dashboard") || isActive("/staff-dashboard")
                    ? "#2563EB"
                    : "transparent",
                color:
                  isActive("/admin-dashboard") || isActive("/staff-dashboard")
            ...
            {role === "admin" && (
              <Link
                ...
                style={{
                  ...
                  padding: "0.5rem 1rem",
                  borderRadius: "8px",
                  background: isActive("/logs") ? "#2563EB" : "transparent",
